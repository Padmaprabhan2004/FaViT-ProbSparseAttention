...
##-------------modify in_chans, num_classes respectively--------------------####
class FactorizationTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=100, embed_dims=[64, 128, 256, 512],
...
"""To test:
Add weight decay in AdamW,
Learning rate scheduling
Add blocks for CIFAR100,IMAGENET100K
"""
...
model = favit_b0(num_classes=num_classes)
...
def cosine_lr_schedule(epoch):
    ...
    elif epoch > epochs - cooldown_epochs:
        return 0.1
    else:
        cos_inner = (epoch - warmup_epochs) / (epochs - warmup_epochs - cooldown_epochs)
...
for epoch in range(epochs):
    ...
    print(f"[Epoch {epoch+1:03d}] Train Loss: {train_loss:.4f}, Acc: {train_acc*100:.2f}% | "
          f"Test Loss: {test_loss:.4f}, Acc: {test_acc*100:.2f}%, LR: {scheduler.get_last_lr()[0]:.6f}")
